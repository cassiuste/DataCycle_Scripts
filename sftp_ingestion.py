...
logging.info("Starting SFTP Ingestion")
...
try:
    ...
    logging.info(f"Checking remote directory: {remote_dir}...")
        ...
        for file_name in files:
            if file_name.startswith("Pred_") and file_name.endswith(".csv"):
                ...
                if not os.path.exists(local_file_path):
                    logging.info(f"Downloading new file: {local_folder}/{year}/{month}/{day}/{file_name}")
                    sftp.get(file_name, local_file_path)
                    total_downloaded += 1

    except IOError:
        logging.warning(f"Could not find or access remote folder '{remote_dir}'.")

    sftp.close()
    transport.close()

    logging.info("Status: Success")
    logging.info(f"New files downloaded: {total_downloaded}")

except Exception as e:
    logging.error(f"Error during SFTP connection or transfer: {e}")